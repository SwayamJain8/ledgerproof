...
import { ConfirmOrderButton, CreateBillButton } from "./order-actions";
...
export default async function PurchaseOrderPage({
  ...
  const order = await prisma.purchaseOrder.findUnique({
    ...
    include: {
      ...
      bills: {
        ...
        select: { id: true, name: true, billDate: true, state: true, totalPaise: true },
      },
    },
  });
  ...
  const displayName = isDraftPlaceholder(order.name) ? "Draft purchase order" : order.name;

  // What is still convertible. Drives both the remainder column and whether
  // the Create Bill action is offered at all.
  const outstanding = order.lines.reduce(
    (sum, line) => sum + (line.quantityMilli - line.qtyBilledMilli),
    0n,
  );
  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-wide text-ink-3">Purchase</p>
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          {!isDraft && outstanding > 0n ? (
            <CreateBillButton orderId={order.id} defaultBillDate={toIsoDate(today())} />
          ) : null}
        </div>
      </div>
      ...
      <div className="space-y-4">
        <Panel className="p-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Detail label="Vendor">{order.vendor.name}</Detail>
          ...
          <Table>
            ...
            <tbody>
              {order.lines.map((line) => {
                const remaining = line.quantityMilli - line.qtyBilledMilli;
                return (
                  <tr key={line.id}>
            ...
            <Table>
              ...
              <tbody>
                {order.bills.map((bill) => (
                  <tr key={bill.id}>
                    <Td>
                      <Link href={`/bills/${bill.id}`} className="text-walnut hover:underline">
                        {isDraftPlaceholder(bill.name) ? "Draft bill" : bill.name}
                      </Link>
                    </Td>
